...
public class GazetteerParserTest {
	...
	@Test(expected = IOException.class) public void parserFailsOnNonExistantFile()
			throws IOException {
		readSummitsFrom(new File("/   /d:/ééé"));
	}

	@Test(expected = IOException.class) public void parserFailsOnGarbageLine()
			throws IOException {
		readSummitsFrom(tempFileWithLines("blabla"));
	}

	@Test(expected = IOException.class) public void parserFailsOnInvalidLongitude()
			throws IOException {
		String l = "  7:25:1x 45:08:25  1325  R0 E07 BA MONTE CURT";
		readSummitsFrom(tempFileWithLines(l));
	}

	@Test(expected = IOException.class) public void parserFailsOnInvalidLatitude()
			throws IOException {
		String l = "  7:25:12 45:08:2_  1325  R0 E07 BA MONTE CURT";
		readSummitsFrom(tempFileWithLines(l));
	}

	@Test(expected = IOException.class) public void parserFailsOnInvalidElevation()
			throws IOException {
		String l = "  7:25:12 45:08:25  leet  R0 E07 BA MONTE CURT";
		readSummitsFrom(tempFileWithLines(l));
	}

	@Test(expected = UnsupportedOperationException.class) public void summitListIsUnmodifiable()
			throws IOException {
		String l = "  7:01:02 46:32:56  2002  H1 B01 D7 LE MOLESON";
		readSummitsFrom(tempFileWithLines(l)).clear();
	}

	@Test public void parserWorksOnValidFile() throws IOException {
		List<Summit> summits = Arrays
				.asList(new Summit("A MONT UN", hmsPoint(7, 30, 00, 15, 16, 17),
								30), new Summit("B MONT ZWEI",
								hmsPoint(6, 12, 34, 1, 23, 45), 10),
						new Summit("C MONTE TRE",
								hmsPoint(1, 33, 33, 15, 66, 66), 1000),
	...
	private String formatSummit(Summit s) {
		...
		return String
				.format("%3d:%02d:%02d %2d:%02d:%02d %5d  R0 E07 BA %s", h(lon),
						m(lon), s(lon), h(lat), m(lat), s(lat), s.elevation(),
						s.name());
	}
}
